...
export default function AnalyticsPage() {
  ...
  async function fetchData() {
    ...
    try {
      const params = {
        ...
        network: selectedNetworks.length > 0
          ? selectedNetworks.join(',')
          : "Instagram,Facebook,LinkedIn,X" // Exclude Google and Chatbot by default
      };
      ...
      setFetchError("No se pudieron cargar los datos. Verifica tu conexión o intenta nuevamente.");
    } finally {
      setLoading(false);
  ...
  const stats = [
    { name: "Total Comentarios", value: summary?.total_comments || 0, icon: MessageSquare, color: "text-sky-600", bg: "bg-sky-50", growth: summary?.growth?.total },
    ...
    { name: "Redes Activas", value: summary?.active_networks || 0, icon: Share2, color: "text-emerald-600", bg: "bg-emerald-50", growth: summary?.growth?.networks },
    { name: "Promedio Comentarios/Día", value: summary?.avg_per_day || 0, icon: TrendingUp, color: "text-rose-600", bg: "bg-rose-50", growth: summary?.growth?.avg },
  ];

  return (
    <div className="space-y-8 pb-12">
      {fetchError && (
        <div className="bg-rose-50 border border-rose-200 text-rose-700 px-5 py-4 rounded-2xl text-sm font-semibold flex items-center gap-3">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="heading-xl">Analytics de Redes Sociales</h1>
          <p className="body-md mt-2 text-[var(--color-text-muted)]">Panel unificado con métricas y tendencias del periodo.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {/* Dedicated Google Section */}
        <motion.div
          initial={{ opacity: 0, scale: 0.98 }}
          animate={{ opacity: 1, scale: 1 }}
          className="card p-8 shadow-sm bg-gradient-to-br from-white to-amber-50/20"
        >
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-bold text-[var(--color-primary-900)] flex items-center gap-2 font-heading">
              <Search className="w-5 h-5 text-amber-600" />
              Explorador Google Maps
            </h2>
            {summary?.total_comments > 0 && distribution?.networks?.find((n: any) => n.name === 'Google') && (
          ...
          <div className="space-y-4">
            <p className="text-sm text-slate-500 italic">
              Sección dedicada para el análisis de reseñas extraídas mediante el barrido de Google Maps.
            </p>

            <button
              onClick={() => {
                setSelectedNetworks(["Google"]);
                setActivePreset("all");
              }}
              className="w-full py-4 border-2 border-dashed border-amber-200 rounded-2xl flex flex-col items-center justify-center gap-2 hover:bg-amber-50 hover:border-amber-300 transition-all text-amber-700 font-bold group"
            >
              <Sparkles className="w-6 h-6 group-hover:scale-110 transition-transform" />
              <span className="text-sm">Filtrar solo Reseñas de Google</span>
            </button>

            <div className="p-4 bg-white rounded-xl border border-amber-100 shadow-sm mt-2">
              <div className="flex items-center justify-between mb-3 text-[10px] font-black uppercase text-slate-400">
                <span>Reseñas Scrapeadas</span>
                <span className="text-amber-600 font-black">{distribution?.networks?.find((n: any) => n.name === 'Google')?.value || 0}</span>
              </div>
              <p className="text-xs text-slate-400">
                Estas reseñas han sido clasificadas automáticamente por temas al igual que tus fuentes de Sprout.
              </p>
            </div>
          </div>
        </motion.div>
